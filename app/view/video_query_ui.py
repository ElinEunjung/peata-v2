...
class VideoQueryUI(QWidget):
    ...
    def connect_live_query_signals(self):
        ...
        self.start_date.dateChanged.connect(lambda: (self.update_query_preview()))
        self.end_date.dateChanged.connect(lambda: (self.update_query_preview()))
    ...
    def create_filter_builder_panel(self):
        ...
        self.logic_groups = {"AND": None, "OR": None, "NOT": None}
    ...
    def validate_date_range(self):
        ...
        if start.daysTo(end) > max_days:
            QMessageBox.warning(
                self,
                "Invalid Date",
                f"End date must be within {max_days} days of start date.",
            )
    ...
    def run_advanced_query(self):
        if not self.has_selected_fields():
            QMessageBox.warning(
                self,
                "Missing Fields",
                "Please select at least one field to include in the result.",
            )
            return
