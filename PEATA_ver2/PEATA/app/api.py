...
class TikTokApi:
    ...
    def fetch_videos_query(
        self, query_body, start_date, end_date, cursor=0, limit=100, search_id=None
    ):
        ...
        if not fields or not isinstance(fields, list) or len(fields) == 0:
            raise ValueError(
                "❗ 'fields' are required in query_body and must contain at least one field."
            )
        ...
        try:
            ...
            response = requests.post(
                self.VIDEO_QUERY_URL, json=body, params=query_params, headers=headers
            )
            ...
            if response.status_code == 200:
                ...
                # rasie error
                user_friendly = get_friendly_error_message(
